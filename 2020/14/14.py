...
def main(data):
   ...
   for row in data:
      if "mask" in row:
         ...
         mem[int(row.split("[")[1].split("]")[0])] = applyBitmask(mask, bin(int(row.split(" = ")[1]))[2:].zfill(36))
         for addr in applyBitmaskV2(mask, bin(int(row.split("[")[1].split("]")[0]))[2:].zfill(36)):
            mem2[addr] = int(row.split(" = ")[1])
   res = 0
   for val in mem.values():
      res += val
   print "Q1: The sum of all values in memory is: {}".format(res)
   res = 0
   for val in mem2.values():
      res += val
   print "Q2: The sum of all values in memory is: {}".format(res)

def applyBitmaskV2(mask, data):
   ...
   for i in range(len(data)):
      if mask[i] == "X":
         data[i] = "X"
         floatingAddr.append(i)
      elif mask[i] == "1":
         data[i] = "1"
   for i in range(2**len(floatingAddr)):
      changes.append(bin(i)[2:].zfill(len(floatingAddr)))
   for change in changes:
      tmp = copy.deepcopy(data)
      for i in range(len(change)):
         tmp[floatingAddr[i]] = change[i]
      res.append(int(''.join(tmp), 2))
   return res

def applyBitmask(mask, data):
   mask = [char for char in mask]
   data = [char for char in data]
   for i in range(len(data)):
      if mask[i] == "0":
