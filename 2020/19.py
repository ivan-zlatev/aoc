...
def main(data, Q1 = True):
   rules = dict()
   for row in data[0]:
      ruleNum = int(row.split(':')[0])
      ruleVal = row.split(': ')[1]
      if 'a' in ruleVal or 'b' in ruleVal:
         ruleVal = ruleVal.replace('"', '')
      elif '|' in ruleVal:
         ruleVal = ruleVal.split(' | ')
         for i in range(len(ruleVal)):
            tmpVal = ruleVal[i].split(' ')
            resList = []
            for j in range(len(tmpVal)):
               resList.append(int(tmpVal[j]))
            ruleVal[i] = resList
      else:
         tmpVal = ruleVal.split(' ')
         resList = []
         for j in range(len(tmpVal)):
            resList.append(int(tmpVal[j]))
         ruleVal = resList
      rules[ruleNum] = ruleVal
   if Q1:
      passRegex = '^' + evaluateRule(rules, 0) + '$'
      results = 0
      for row in data[1]:
         if re.search(passRegex, row):
            results += 1
      print "Q1: {}".format(results)
   else:
      passRegex = '^' + evaluateRuleQ2(rules, 0) + '$'
      results = 0
      for row in data[1]:
         if re.search(passRegex, row):
            results += 1
      ...
      if isinstance(rules[rule][0], int):
         ...
         for i in range(len(rules[rule])):
            ...
            if i < len(rules[rule])-1:
               ...
               result = result + '))'
            #print result
         #print rules[rule], rules[rule][0], type(rules[rule][0]), isinstance(rules[rule][0], int)
         return result

def evaluateRule(rules, rule):
   if isinstance(rules[rule], str):
      return rules[rule]
   if isinstance(rules[rule], list):
      if isinstance(rules[rule][0], int):
         result = ''
         for i in range(len(rules[rule])):
            result = result + evaluateRule(rules, rules[rule][i])
         return result
      else:
         result = ''
         for i in range(len(rules[rule])):
