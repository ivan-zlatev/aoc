...
def main(data, bags):
   ...
   bags = [bags]
   outerBags = calculateOuterBags(rules, bags, [])
   print "Q1: outer bags that can contain {}: {}\n".format(bags, outerBags)
   bags = [{bags[0]:1}]
   innerBags = calculateInnerBags(rules, bags)
   print "Q2: inner bags that are required in {}: {}\n".format(bags, innerBags)

def calculateOuterBags(rules, bags, alreadyUsed):
   validRules = []
   for bag in bags:
      for key in rules.keys():
         if rules[key] and bag in rules[key].keys():
            if key not in alreadyUsed:
               validRules.append(key)
   validRules = set(validRules)
   for rule in validRules:
      alreadyUsed.append(rule)
   if len(validRules) == 0:
      return 0
   else:
      return len(validRules) + calculateOuterBags(rules, validRules, alreadyUsed)

def calculateInnerBags(rules, bags, count = 0):
   requiredBags = []
   for outerBag in bags:
      outerCount = int(outerBag.values()[0])
      outerBag = outerBag.keys()[0]
      for i in range(outerCount):
         if rules[outerBag]:
            for innerBag, innerBagCount in rules[outerBag].items():
               innerBag = {innerBag:innerBagCount}
               requiredBags.append(innerBag)
   if len(requiredBags) == 0:
      return 0
   else:
      #print requiredBags
      for bag in requiredBags:
         count += int(bag.values()[0])
      #print count
      return count + calculateInnerBags(rules, requiredBags)

def parseData(data):
   rules = {}
   for row in data:
      bag = row.split(" bags ")[0]
      contents = row.split(" contain ")[1][:-1]
      if not contents == "no other bags":
         contents = contents.split(", ")
         bagContents = {}
         for bags in contents:
            bagsNum = bags.split(" ")[0]
            bagsColor = bags.split(" ")[1] + " " + bags.split(" ")[2]
            bagContents[bagsColor] = bagsNum
...
#main(inputData, "shiny gold")
#main(inputData, "faded blue")
#main(inputData, "doted balck")
