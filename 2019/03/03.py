...
def main(paths):
   ...
   steps2 = []
   distanceMin = 1000000
   print "[{}] Evaluating path1".format(time.ctime())
   for path in path1:
      if path[0] == 'U':
         for i in range(int(path.split('U')[1])):
            coords[1] += 1
            steps += 1
            if coords[:] not in steps1:
               steps1.append([coords[:],steps])
      elif path[0] == 'D':
         for i in range(int(path.split('D')[1])):
            coords[1] -= 1
            steps += 1
            if coords[:] not in steps1:
               steps1.append([coords[:],steps])
      elif path[0] == 'L':
         for i in range(int(path.split('L')[1])):
            coords[0] -= 1
            steps += 1
            if coords[:] not in steps1:
               steps1.append([coords[:],steps])
      elif path[0] == 'R':
         for i in range(int(path.split('R')[1])):
            coords[0] += 1
            steps += 1
            if coords[:] not in steps1:
               steps1.append([coords[:],steps])
      else:
         print "Error evaluating the path: [{}]".format(path)
   coords = [0,0]
   steps = 0
   print "[{}] Evaluating path2".format(time.ctime())
   for path in path2:
      if path[0] == 'U':
         for i in range(int(path.split('U')[1])):
            coords[1] += 1
            steps += 1
            if coords[:] not in steps2:
               steps2.append([coords[:],steps])
      elif path[0] == 'D':
         for i in range(int(path.split('D')[1])):
            coords[1] -= 1
            steps += 1
