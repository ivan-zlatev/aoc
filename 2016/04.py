...
def main(strings):
   ...
   for s in strings:
      ...
      checksum = [tmp for tmp in s.split('[')[1].split(']')[0]]
      #print s, data, num, checksum
      weight = len(data)
      failed = False
      tmp    = []
      alpha  = False
      for c in checksum:
         new = len(re.findall(c, data))
         tmp.append([c,new])
         if new == 0:
            #print 'Failed for missing character'
            failed = True
            break
         elif weight < new:
            #print 'Failed for char weight: {}'.format(data)
            failed = True
            break
         elif weight == new:
            alpha = True
         weight = new
      if alpha:
         print 'same weight, needs more checking {}'.format(data)
      if not failed and not alpha:
         print '{} passed'.format(data)
      else:
