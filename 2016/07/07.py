      ...
      for i in range(len(s)-3):
         if s[i] == '[':
            ...
            if hyperNet < 1:
               firstChar = s[i]
               secondChar = s[i+1]
               if not firstChar == secondChar and not TLSi:
                  if firstChar == s[i+3] and secondChar == s[i+2]:
                     #print 'TLS found[{}{}{}{}]: {}'.format(firstChar,secondChar,s[i+2],s[i+3],s)
                     TLS = True
                     #print '\t\t\t TLS found'
            else:
               firstChar = s[i]
               secondChar = s[i+1]
               if not firstChar == secondChar:
                  if firstChar == s[i+3] and secondChar == s[i+2]:
                     TLSi = True
                     if TLS:
                        TLS = False
         ...
         #for j in range(i):
         #   print '',
         #print '{}{}{}{}'.format(s[i],s[i+1],s[i+2],s[i+3])
      if TLS:
         counter += 1
   print 'IP addresses supporting ABBA: {}'.format(counter)
   counter = 0
   for s in strings:
      #print ''
      #print '------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------'
      ...
      for i in range(len(s)-2):
         if s[i] == '[':
            ...
            if hyperNet < 1: # search for ABA strings
               firstChar = s[i]
               secondChar = s[i+1]
               thirdChar = s[i+2]
               if firstChar == thirdChar and not firstChar == secondChar and secondChar not in ['[',']'] and thirdChar not in ['[',']']:
                  if [secondChar, firstChar, secondChar] not in ABA:
                     ABA.append([secondChar, firstChar, secondChar])
                     #print '\t\t\t ABA found'
         #for j in range(i):
         #   print '',
         #print '{}{}{}'.format(s[i],s[i+1],s[i+2])
      SSL = False
      if len(ABA) > 0:
         #print ABA
         hyperNet = 0
         for i in range(len(s)-2):
            if s[i] == '[':
               ...
               if hyperNet > 0: # search for ABA strings
                  firstChar = s[i]
                  secondChar = s[i+1]
                  thirdChar = s[i+2]
                  if firstChar == thirdChar and not firstChar == secondChar:
                     if [firstChar, secondChar, thirdChar] in ABA:
                        SSL = True
                        #print '\t\t\t SSL found'
            #for j in range(i):
            #   print '',
      ...
      #else:
      #   print s + ' failes'
   print 'IP addresses supporting SSL: {}'.format(counter)
